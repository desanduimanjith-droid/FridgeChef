...
export function IngredientInput({ onSearch }: { onSearch: (tags: string[]) => void }) {
  ...
  return (
    <div className="mx-auto mt-10 max-w-3xl">
      <div className="rounded-[2rem] border border-white/70 bg-white/85 p-4 shadow-[0_24px_80px_rgba(15,23,42,0.12)] backdrop-blur-xl">
          ...
          <AnimatePresence>
            {ingredients.map((ingredient, index) => (
              <motion.div
                ...
                className="flex items-center gap-3 rounded-[1.2rem] border border-emerald-100 bg-emerald-50/35 px-4 py-3"
              >
                ...
                {index === ingredients.length - 1 ? (
                  <button
                    ...
                    className="shrink-0 rounded-full bg-emerald-600 p-2.5 text-white transition-colors hover:bg-emerald-700"
                  >
                  ...
                  <button
                    ...
                    className="shrink-0 rounded-full bg-white/90 p-2.5 text-muted-500 transition-colors hover:text-brand-500"
                  >
                    <X className="h-4 w-4" />
                  </button>
                )}
